...
这些 term 在当前游戏资源中不存在，普通生成入口会提示风险并继续生成。v2.4.1 新增文本目前可能仍保持原文，零散漏翻会先收集，等内容积累多一些后再集中更新翻译表。
...
- Issue #1：`[Feature]: 5月28日 v2.4.1 版本更新`
  - 已回复：普通生成入口已支持跳过当前版本缺失的旧 term 并继续生成；已说明生成体积、Python 3.13 检测、v2.4.1 漏翻收集节奏和繁体中文计划。
  - 后续：零散漏翻先收集，等内容积累多一些后再集中更新翻译表；繁体中文后续有时间可能尝试，但维护者近期毕业事务较忙，且繁体字体和显示细节还没有系统测试。
